describe('MovieListPage - Complete Navigation and Functionality', () => {
  ...
  describe('Initial Page Load', () => {
    it('should display the main page elements', () => {
      ...
      cy.get('[data-cy="add-movie-btn"]')
        .should('be.visible')
        .and('contain.text', '+ ADD MOVIE');
    ...
    it('should navigate to add movie form when add movie button is clicked', () => {
      cy.get('[data-cy="add-movie-btn"]').click();

      cy.url().should('include', '/new');
      cy.get('[data-cy="dialog"]').should('be.visible');
      cy.get('[data-cy="dialog"]').should('contain.text', 'ADD MOVIE');
      cy.get('[data-cy="movie-form"]').should('be.visible');
    });

    it('should navigate to edit movie form when edit button is clicked', () => {
      // Intercept the movie details API call
      ...
      cy.url().should('match', /\/\d+\/edit/);

      // Wait for the movie details API call to complete
      cy.wait('@getMovieDetails');

      // Wait for the dialog to appear
      cy.get('[data-cy="dialog"]', { timeout: 10000 }).should('be.visible');
      cy.get('[data-cy="dialog"]').should('contain.text', 'EDIT MOVIE');

      // Wait for the form to appear
      cy.get('[data-cy="movie-form"]', { timeout: 10000 }).should('be.visible');
    });
  });
  ...
  describe('Search Functionality', () => {
    it('should perform search and display results', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?search=Bohemian&searchBy=title&sortBy=releaseDate&sortOrder=desc',
        {
      ...
      cy.get('[data-cy^="movie-tile-"]')
        .first()
        .should('contain.text', 'Bohemian Rhapsody');
    });
  ...
  describe('Genre Filtering', () => {
    ...
    it('should filter movies by Horror genre', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?filter=HORROR&sortBy=releaseDate&sortOrder=desc',
        {
          body: { data: [] },
        }
      ).as('getHorrorMovies');
    ...
    it('should filter movies by Crime genre', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?filter=CRIME&sortBy=releaseDate&sortOrder=desc',
        {
          body: { data: [] },
        }
      ).as('getCrimeMovies');
    ...
    it('should filter movies by Documentary genre', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?filter=DOCUMENTARY&sortBy=releaseDate&sortOrder=desc',
        {
          body: { data: [] },
        }
  ...
  describe('Movie Selection and Details', () => {
    it('should show movie details when a movie is clicked', () => {
      ...
      cy.get('[data-cy="back-btn"]')
        .should('be.visible')
        .and('contain.text', '← Back to search');
  ...
  describe('Loading States', () => {
    it('should show loading state during API calls', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?search=Loading&searchBy=title&sortBy=releaseDate&sortOrder=desc',
        {
          delay: 1000,
          body: { data: [] },
        }
  ...
  describe('Error Handling', () => {
    it('should handle API errors gracefully', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?search=Error&searchBy=title&sortBy=releaseDate&sortOrder=desc',
        {
          statusCode: 500,
    ...
    it('should handle empty search results', () => {
      cy.intercept(
        'GET',
        'http://localhost:4000/movies?search=NonexistentMovie&searchBy=title&sortBy=releaseDate&sortOrder=desc',
        {
          body: { data: [] },
        }
      ).as('getEmptyResults');
